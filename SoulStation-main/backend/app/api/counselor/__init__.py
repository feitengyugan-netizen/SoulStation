...
@router_appointment.delete("/{appointment_id}", summary="删除预约订单")
async def delete_appointment(
    ...
    try:
        ...
        # 验证权限：只能是订单的用户或对应的咨询师
        is_owner = appointment.user_id == user_id
        is_counselor = False

        # 检查是否是咨询师
        if appointment.counselor and appointment.counselor.user_id:
            is_counselor = appointment.counselor.user_id == user_id
        ...
        # 只允许删除已完成或已取消的订单
        if appointment.status not in ['completed', 'cancelled', 'refunded']:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"订单状态为{appointment.status}，无法删除。请先取消订单。"
            )
        ...
        try:
            ...
            messages = db.query(ConsultationMessage).filter(
                ConsultationMessage.appointment_id == appointment.id
            ).all()
...
@router.post("/{counselor_id}/publish", summary="发布咨询师（管理员）")
async def publish_counselor(
    counselor_id: int,
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        if not counselor.is_verified:
            raise ValueError("咨询师未通过认证，无法发布")
        ...
        logger.error(f"发布咨询师失败: {str(e)}")
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"发布咨询师失败: {str(e)}"
        )


@router.post("/{counselor_id}/unpublish", summary="下架咨询师（管理员）")
async def unpublish_counselor(
    counselor_id: int,
    reason: Optional[str] = Query(None, description="下架原因"),
    db: Session = Depends(get_db)
):
    """
    下架咨询师信息（管理员专用）

    将咨询师状态设置为 inactive，使其不在前端显示
    但不删除数据，可以重新发布
    """
    try:
        from app.models.counselor import Counselor
        ...
        logger.error(f"下架咨询师失败: {str(e)}")
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"下架咨询师失败: {str(e)}"
        )


@router.get("/home/featured", summary="获取首页推荐咨询师")
async def get_featured_counselors(
    ...
    try:
        ...
        counselors = db.query(Counselor).filter(
            ...
            desc(Counselor.rating),
            desc(Counselor.review_count)
        ).limit(limit).all()
        ...
        logger.error(f"获取推荐咨询师失败: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"获取推荐咨询师失败: {str(e)}"
        )
