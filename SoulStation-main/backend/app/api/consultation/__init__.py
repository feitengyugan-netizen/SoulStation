...
# 导入评价路由
from app.api.consultation.reviews import router as reviews_router
# 导入用户订单路由
from app.api.consultation.user_orders import router as user_orders_router
...
@router.get("/{appointment_id}/detail", summary="获取预约详情")
async def get_appointment_detail(
    ...
    try:
        ...
        # 权限验证：用户只能查看自己的预约，咨询师只能查看分配给自己的预约
        if user_type == 'user':
...
async def get_messages(
    ...
    try:
        ...
        result = ConsultationService.get_messages(
            db, appointment_id, sender_id, user_type, last_id, limit
        )
        return {
            "code": 200,
            "message": "获取成功",
...
async def send_message(
    ...
    try:
        ...
        message = ConsultationService.send_message(
            db, appointment_id, sender_id, user_type, message_data
        )
        return {
            "code": 200,
            "message": "发送成功",
...
async def end_consultation(
    appointment_id: int,
    summary: Optional[str] = Body(None, description="咨询总结（咨询师可选）"),
    suggestions: Optional[str] = Body(None, description="后续建议（咨询师可选）"),
    user_id: int = Depends(get_current_user_info),
    user_type: str = Depends(get_current_user_role),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        ConsultationService.end_consultation(
            db, appointment_id, sender_id, user_type, summary, suggestions
        )
        return {
            "code": 200,
            "message": "咨询已结束，等待用户评价",
            "data": {
