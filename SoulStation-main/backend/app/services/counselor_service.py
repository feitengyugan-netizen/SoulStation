...
class AppointmentService:
        ...
        try:
            ...
            if counselor.user_id:
                try:
                    ...
                    if counselor_user and counselor_user.email:
                        EmailService.send_appointment_confirmation_counselor(
                            email=counselor_user.email,
                            counselor_name=counselor.name,
    ...
    def get_user_appointments(
        ...
        for appt in appointments:
            ...
            frontend_status = status_map.get(appt.status, appt.status)

            # 获取咨询师信息
            counselor_name = "未知咨询师"
            counselor_avatar = None
            counselor_rating = None
            counselor_review_count = None

            if appt.counselor:
                counselor_name = appt.counselor.name
                counselor_avatar = appt.counselor.avatar
                counselor_rating = appt.counselor.rating
                counselor_review_count = appt.counselor.review_count

            # 获取评价信息（从ConsultationReview表）
            rating = None
            review = None
            try:
                from app.models.counselor import ConsultationReview
                review_record = db.query(ConsultationReview).filter(
                    ConsultationReview.appointment_id == appt.id
                ).first()
                if review_record:
                    rating = review_record.rating
                    review = review_record.content
            except Exception:
                pass
            ...
            appt_dict = {
                ...
                "counselorId": appt.counselor_id,
                "counselorName": counselor_name,
                "userId": appt.user_id,
                "userName": appt.user_name,
                "userContact": appt.user_contact,
                "date": appt.appointment_date.strftime("%Y-%m-%d") if appt.appointment_date else "",
                "timeSlot": appt.appointment_date.strftime("%H:%M") if appt.appointment_date else "",
                "type": appt.consultation_type,
                "duration": appt.duration,
                "description": appt.problem_description,
                "price": appt.price,
                "status": frontend_status,
                "rating": rating,
                "review": review,
                "createdAt": appt.created_at.strftime("%Y-%m-%d %H:%M:%S") if appt.created_at else ""
            }

            # 添加完整的咨询师信息
            if appt.counselor:
                appt_dict["counselor"] = {
                    "id": appt.counselor.id,
                    "name": appt.counselor.name,
                    "avatar": appt.counselor.avatar,
                    "title": appt.counselor.title,
                    "rating": counselor_rating or 5.0,
                    "reviewCount": counselor_review_count or 0
                }
    ...
    def cancel_appointment(db: Session, user_id: int, appointment_id: int, reason: Optional[str] = None) -> bool:
        ...
        appointment_data = {
            ...
            "counselor_name": appointment.counselor.name if appointment.counselor else "咨询师",
        ...
        try:
            ...
            appointment_time_str = appointment_data["appointment_date"].strftime("%H:%M")

            # 通知用户
            user = db.query(User).filter(User.id == appointment_data["user_id"]).first()
            if user and user.email:
                try:
            ...
            counselor = db.query(Counselor).filter(Counselor.id == appointment_data["counselor_id"]).first()
...
class ConsultationService:
    ...
    def get_counselor_orders(
        ...
        """获取咨询师订单列表"""
        # 基础查询
        base_query = db.query(Appointment).filter(Appointment.counselor_id == counselor_id)

        # 统计各状态订单数量
        from sqlalchemy import func
        status_counts = {}
        for status in ['pending', 'confirmed', 'in_progress', 'completed']:
        ...
        items = []
        for appt in appointments:
            # 状态映射：数据库中的in_progress转换为前端期望的inprogress
            status_map = {
                "in_progress": "inprogress"
            }
            frontend_status = status_map.get(appt.status, appt.status)
            ...
            if user:
                appt_dict["userName"] = appt_dict["userName"] or user.nickname
    ...
    def get_messages(
        ...
        # 转换为响应格式
        list = [MessageResponse.model_validate(msg) for msg in messages]
    ...
    def end_consultation(
        ...
        # 如果咨询师提供了总结和建议，保存到备注
        if user_type == 'counselor' and (summary or suggestions):
            note_parts = []
            if summary:
                note_parts.append(f"咨询总结: {summary}")
            if suggestions:
                note_parts.append(f"后续建议: {suggestions}")
            appointment.counselor_notes = '\n'.join(note_parts)

        # 发送系统消息 - 通知用户进行评价
        ConsultationService._send_system_message(
            db, appointment_id,
            "咨询已结束，感谢您的使用！请对本此咨询进行评价，帮助我们改进服务质量。"
        )
